    ...
    <>
      <section id="experience" className="py-24 scroll-mt-16">
        ...
        <div className="relative max-w-3xl mx-auto">
          <div className="absolute left-7 w-0.5 bg-gray-700 h-full -translate-x-1/2"></div>
            ...
            <div key={index} className="relative pl-20 mb-12">
              <div className="absolute left-7 top-1 -translate-x-1/2">
                  {item.logo && React.cloneElement(item.logo as React.ReactElement<{ className?: string }>, { 
                      className: 'w-14 h-14 p-1 bg-gray-900 border-2 border-gray-700 rounded-full' 
                  })}
              </div>
              <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                  <div className="flex items-start md:items-center gap-4 flex-col md:flex-row mb-4">
                      <div className="flex-grow">
                          <h3 className="text-xl font-bold text-gray-100">{item.company}</h3>
                          <p className="text-cyan-400 font-semibold">{item.role}</p>
                      </div>
                      <div className="text-sm text-gray-400 md:text-right flex-shrink-0">
                          <p>{item.duration}</p>
                          <p>{item.location}</p>
                      </div>
                  </div>
      ...
      <Modal isOpen={isModalOpen} onClose={closeCaseStudiesModal} title={modalTitle}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {selectedCaseStudies.map((study) => (
            <div key={study.id} className="bg-gray-700/50 rounded-lg p-6 flex flex-col items-start shadow-lg">
                <div className="flex items-center gap-4 mb-4">
                  <span className="text-cyan-400 bg-gray-900/50 p-3 rounded-full">
